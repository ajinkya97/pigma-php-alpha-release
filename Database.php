    ...
    //{
        ...
        function insert() {
            ...
            elseif(count($args)==3 or (count($args)==4 and $args[3]==true)){
                ...
                elseif(is_string($args[1]) and is_array($args[2])){
                    $col = makeString(explode(',',$args[1]),',','');
                    $val = makeString($args[2]);
                    $query = "insert into $table($col) values ($val);";
                }
                elseif(is_array($args[1]) and is_string($args[2])){
        ...
        function update() {
            ...
            if($cnt==2){
            ...
            else if($cnt==3 or ($cnt==4 and $args[3]==true)){
                $changes = "";
                if(is_string($args[1]) and is_string($args[2])){
                    $val = makeString(explode(',',$args[1]));
                    $col = makeString(explode(',',$args[2]),',','');
                }
                else if(is_array($args[1]) and is_string($args[2])){
                    $val = $args[1];
                    $col = makeString(explode(',',$args[2]),',','');
                }
                else if(is_string($args[1]) and is_array($args[2])){
                    $val = makeString(explode(',',$args[1]));
                    $col = $args[2];
                }
                ...
                elseif(is_array($args[3])){
                    $where = makeString($args[3],',','');
                }
                else{
                    $this->error("Invalid Parameters");
                }
                $query = $this->update($table,$args[1],$args[2],true)." $where";
            }
            else{
                return $this->error("invalid parameters");
            }
        ...
        function delete(){
            ...
            else if($cnt==2 or ($cnt==3 and $args[2]==true)){
                $where = $args[1];
                if(is_array($args[1])){
                    $where = makeString($args[1],' and ','');
                }
                $query = $this->update($table,true)." $where";
            }
            else{
                $this->error("Invalid parameters");
            }
    ...
    //{
        ...
        function drop(){
            ...
            if(count($args)==2){
                $type = $args[0];
                $name = $args[1];
                if(is_string($type) and is_array($name)){
